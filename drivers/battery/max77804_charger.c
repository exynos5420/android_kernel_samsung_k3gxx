...
static int max77804_get_byp_dtls(struct max77804_charger_data *charger);
		...
		int cur)
{
	...
	if (reg_data == set_current_reg) {
		...
		while((set_current_reg > (MINIMUM_INPUT_CURRENT / charger->input_curr_limit_step)) && (set_current_reg < 255)) {
			byp_dtls = max77804_get_byp_dtls(charger);
			if ((byp_dtls == 0x08) &&
...
static int max77804_get_byp_dtls(struct max77804_charger_data *charger)
{
	...
	max77804_read_reg(charger->max77804->i2c,
			...
			&reg_data);
	reg_data = ((reg_data & MAX77804_BYP_DTLS) >>
...
#if defined(CONFIG_BATTERY_SWELLING)
static void max77804_set_float_voltage(struct max77804_charger_data *charger, int float_voltage)
{
	u8 reg_data = 0;

	reg_data = max77804_get_float_voltage_data(float_voltage);
	...
	max77804_read_reg(charger->max77804->i2c, MAX77804_CHG_REG_CHG_CNFG_04, &reg_data);
	pr_info("%s: battery cv voltage 0x%x\n", __func__, reg_data);
}

static u8 max77804_get_float_voltage(struct max77804_charger_data *charger)
{
	u8 reg_data = 0;
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		...
		val->intval *= 1000;        // AOSP expects microamperes
		break;
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		val->intval = max77804_get_float_voltage(charger);
		break;
#endif
#ifdef WPC_CHECK_CVPRM_FEATURE
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if (charger->pdata->full_check_type_2nd == SEC_BATTERY_FULLCHARGED_CHGPSY) {
			...
			if (chg_mode.intval == SEC_BATTERY_CHARGING_2ND) {
				max77804_set_charger_state(charger, 0);
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		...
		max77804_set_float_voltage(charger, val->intval);
		break;
#endif
	case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:
...
static void max77804_charger_initialize(struct max77804_charger_data *charger)
{
	...
	/* unmasked: CHGIN_I, BATP_I, BYP_I, (WCIN_I)*/
	if (charger->wc_detect_gpio)
		max77804_write_reg(charger->max77804->i2c,
			MAX77804_CHG_REG_CHG_INT_MASK, 0xba);
	else
		max77804_write_reg(charger->max77804->i2c,
...
static void wpc_detect_work(struct work_struct *work)
{
	...
	if (!chg_data->wc_detect_gpio) {
		// Do unmask again. (for freqeunt wcin irq problem)
		max77804_read_reg(chg_data->max77804->i2c,
			MAX77804_CHG_REG_CHG_INT_MASK, &reg_data);
		if (reg_data & (0x1 << 5)) {
			reg_data &= ~(1 << 5);
...
static int sec_charger_parse_dt(struct max77804_charger_data *charger)
{
	...
	if (np == NULL) {
		...
		charger->wc_detect_gpio = of_property_read_bool(np, "battery,wc_detect_gpio");

		if (charger->wc_detect_gpio) {
			pr_info("%s: Wireless charger detect by GPIO !!!\n", __func__);
			charger->wc_w_gpio = of_get_named_gpio(np,
					"battery,wc_int", 0);
			if (charger->wc_w_gpio < 0) {
				pr_err("%s error reading wc_irq = %d\n", __func__,
					charger->wc_w_gpio);
				charger->wc_w_gpio = 0;
			}
		}
...
*/
	...
	charger->wc_w_irq = charger->wc_detect_gpio ?
			gpio_to_irq(charger->wc_w_gpio) :
			pdata->irq_base + MAX77804_CHG_IRQ_WCIN_I;
	wc_w_irq_attr = charger->wc_detect_gpio ?
	...
	if (charger->wc_detect_gpio) {
		ret = enable_irq_wake(charger->wc_w_irq);
		if (ret < 0)
